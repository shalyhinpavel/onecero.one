...
impl VectorDB {
    ...
    pub async fn search_vector(&self, vector: &[f32], limit: usize) -> Result<Vec<Document>, Box<dyn std::error::Error + Send + Sync>> {
        ...
        while let Some(batch_res) = stream.next().await {
            ...
            let texts = batch.column_by_name("text").unwrap().as_any().downcast_ref::<arrow_array::StringArray>().unwrap();
            let entities = batch.column_by_name("entities").unwrap().as_any().downcast_ref::<arrow_array::StringArray>().unwrap();
            
            let meta_struct = batch.column_by_name("metadata").unwrap().as_any().downcast_ref::<arrow_array::StructArray>().unwrap();
            let fn_array = meta_struct.column_by_name("filename").unwrap().as_any().downcast_ref::<arrow_array::StringArray>().unwrap();
            let t_array = meta_struct.column_by_name("title").unwrap().as_any().downcast_ref::<arrow_array::StringArray>().unwrap();
            let s_array = meta_struct.column_by_name("source").unwrap().as_any().downcast_ref::<arrow_array::StringArray>().unwrap();
            let ts_array = meta_struct.column_by_name("timestamp").unwrap().as_any().downcast_ref::<arrow_array::StringArray>().unwrap();
